...
def gs_path_parse(sgfullpath):
  # START post-processing eventdata. Now assume that we have bucketname,
  # dirinbucket, sgfullpath is a url so we use the urlparse to analyze it
  try:
    ...
    # we will have ParseResult(scheme='gs', netloc='bachphuresultsamples',
    # path='/Alarm/Linh/aaa', params='', query='', fragment='')
...
def copy_dir(google_storage_link, dest_temp_dir):
  ...
  # we assume that the guy who calls this function knows how to prepare the
  # right dir
  ...
  for blob in blobs:
    ...
    if (blob.name.find(inbucket_dir) > -1):
      if not (blob.name.endswith("/")):
        ...
        # here you can also filter othe files, e.g., do not download
        # _SUCCESS
